...
def test_vague_usage_asks_correct_question_without_writing(client, db):
    milk = PantryItem(user_id=uuid.uuid4(), canonical_name=sf("milk"), quantity_type="capacity_bucket", quantity_value=sf("full"), status="stored")
    tomatoes = PantryItem(user_id=uuid.uuid4(), canonical_name=sf("tomatoes"), quantity_type="count", quantity_value=sf(6), status="stored")
